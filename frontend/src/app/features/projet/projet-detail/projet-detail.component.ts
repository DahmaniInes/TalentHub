...
export class ProjetDetailComponent implements OnInit, OnDestroy {
  ...
  private projetSvcLocal = inject(ProjetService);
statutsProjet = signal<StatutProjet[]>([]);
  ...
  ngOnInit(): void {
    ...
    this.projetSvc.getById(id).subscribe({
      next: p => {
        ...
        this.projetSvcLocal.getStatutsProjet().subscribe({
  ...
  createActiviteForProjet(): void {
    if (!this.perms.canCreateActivity() && !this.perms.canEditAnyProject()) {
      this.ui.warning('Permission requise.');
      return;
    }
    ...
    this.activiteSvc.create(f).subscribe({
      next: newActivite => {
        ...
        this.projetSvc.assignerActivites(p.id, [...currentIds, newActivite.id]).subscribe({
          next: () => {
            this.ui.success('Activité créée et assignée.');
          ...
          error: () => { this.ui.error('Activité créée mais erreur assignation.'); this.submittingActivite.set(false); this.loadActivites(p.id); }
        });
      },
  ...
  // ── Helpers statut projet via nomenclature ──
