// src/app/features/activites/activite-detail/activite-detail.component.ts
...
export class ActiviteDetailComponent implements OnInit {
    ...
    activeTab = signal<'infos' | 'commentaires'>('infos');
    ...
    ngOnInit(): void {
        ...
        this.activiteSvc.getById(id).subscribe({
            next: a => { this.activite.set(a); this.loading.set(false); this.loadCommentaires(id); },
    ...
    deleteComment(c: Commentaire): void {
        this.ui.confirm({ title: 'Supprimer', message: 'Supprimer ce commentaire ?', type: 'danger', confirmLabel: 'Supprimer',
            onConfirm: () => {
    ...
    getStatutBadgeClass(id?: number): string {
        ...
        const map: Record<string, string> = { 'A_FAIRE': 'dt-badge dt-status-a-faire', 'EN_COURS': 'dt-badge dt-status-en-cours', 'EN_REVUE': 'dt-badge dt-status-en-revue', 'TERMINE': 'dt-badge dt-status-termine', 'BLOQUE': 'dt-badge dt-status-bloque', 'ANNULE': 'dt-badge dt-status-annule' };
    ...
    getProgressColor(p?: number, e?: number): string { const pct = this.getProgressPct(p,e); return pct>=100?'#ef4444':pct>=80?'#f59e0b':'#10b981'; }
    getAvatarColor(name: string): string { const c=['#6366f1','#8b5cf6','#ec4899','#ef4444','#f97316','#10b981','#06b6d4','#3b82f6']; return c[(name||'').charCodeAt(0)%c.length]; }
    getInitiales(n: string): string { return (n||'').split(' ').slice(0,2).map(w=>w.charAt(0).toUpperCase()).join(''); }
    fmtDate(d?: string): string { if(!d) return '—'; const dt=new Date(d); const m=['Jan','Fév','Mar','Avr','Mai','Jun','Jul','Aoû','Sep','Oct','Nov','Déc']; return `${String(dt.getDate()).padStart(2,'0')} ${m[dt.getMonth()]}, ${dt.getFullYear()}`; }
    fmtDateTime(d?: string): string { if(!d) return '—'; const dt=new Date(d); const m=['Jan','Fév','Mar','Avr','Mai','Jun','Jul','Aoû','Sep','Oct','Nov','Déc']; return `${String(dt.getDate()).padStart(2,'0')} ${m[dt.getMonth()]} ${dt.getFullYear()} à ${String(dt.getHours()).padStart(2,'0')}:${String(dt.getMinutes()).padStart(2,'0')}`; }
