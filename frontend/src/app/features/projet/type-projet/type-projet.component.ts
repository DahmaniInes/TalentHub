...
export class TypeProjetComponent implements OnInit {
  ...
  private svc    = inject(TypeProjetService);
  readonly ui    = inject(UiService);
  readonly perms = inject(PermissionContextService);
  private fb     = inject(FormBuilder);

  items       = signal<TypeProjet[]>([]);
  ...
  ngOnInit(): void {
    if (!this.perms.canViewTypeProjet()) return;
    this.loadAll();
  }
  ...
  openCreate(): void {
    if (!this.perms.canCreateTypeProjet()) { this.ui.warning('Permission requise.'); return; }
  ...
  openEdit(item: TypeProjet): void {
    if (!this.perms.canEditTypeProjet()) { this.ui.warning('Permission requise.'); return; }
  ...
  delete(item: TypeProjet): void {
    if (!this.perms.canDeleteTypeProjet()) { this.ui.warning('Permission requise.'); return; }
    this.ui.confirm({
  ...
  toggleActif(item: TypeProjet, e: Event): void {
    e.stopPropagation();
    if (!this.perms.canEditTypeProjet()) { this.ui.warning('Permission requise.'); return; }
