...
export class StatutProjetComponent implements OnInit {
  ...
  private svc    = inject(StatutProjetService);
  readonly ui    = inject(UiService);
  readonly perms = inject(PermissionContextService);
  private fb     = inject(FormBuilder);

  items       = signal<StatutProjet[]>([]);
  ...
  ngOnInit(): void {
    if (!this.perms.canViewStatutProjet()) return;
    this.loadAll();
  }
  ...
  openCreate(): void {
    if (!this.perms.canCreateStatutProjet()) { this.ui.warning('Permission requise.'); return; }
    this.editingId.set(null);
    this.form.reset({ code: '', libelle: '', couleur: '#6366f1', ordreAffichage: 0, actif: true });
    this.form.get('code')?.enable();
    this.slideOpen.set(true);
  }

  openEdit(item: StatutProjet): void {
    if (!this.perms.canEditStatutProjet()) { this.ui.warning('Permission requise.'); return; }
  ...
  delete(item: StatutProjet): void {
    if (!this.perms.canDeleteStatutProjet()) { this.ui.warning('Permission requise.'); return; }
    this.ui.confirm({
  ...
  toggleActif(item: StatutProjet, e: Event): void {
    e.stopPropagation();
    if (!this.perms.canEditStatutProjet()) { this.ui.warning('Permission requise.'); return; }
