...
@Component({
  selector: 'app-statut-stage',
  standalone: true,
  imports: [CommonModule, FormsModule, ReactiveFormsModule],
  templateUrl: './statut-stage.component.html'
})
export class StatutStageComponent implements OnInit {

  private svc    = inject(StatutStageService);
  readonly ui    = inject(UiService);
  readonly perms = inject(PermissionContextService);
  private fb     = inject(FormBuilder);

  items       = signal<StatutStage[]>([]);
  ...
  ngOnInit(): void {
    if (!this.perms.canViewStatutStage()) return;
    this.loadAll();
  }
  ...
  openCreate(): void {
    if (!this.perms.canCreateStatutStage()) { this.ui.warning('Permission requise.'); return; }
    this.editingId.set(null);
    this.form.reset({ code: '', libelle: '', couleur: '#8b5cf6', ordreAffichage: 0, actif: true });
    this.form.get('code')?.enable();
    this.slideOpen.set(true);
  }

  openEdit(item: StatutStage): void {
    if (!this.perms.canEditStatutStage()) { this.ui.warning('Permission requise.'); return; }
    this.editingId.set(item.id);
    this.form.patchValue({
      code: item.code, libelle: item.libelle,
      couleur: item.couleur || '#8b5cf6',
      ordreAffichage: item.ordreAffichage || 0, actif: item.actif
    });
  ...
  delete(item: StatutStage): void {
    if (!this.perms.canDeleteStatutStage()) { this.ui.warning('Permission requise.'); return; }
    this.ui.confirm({
  ...
  bulkDelete(): void {
    ...
    this.ui.confirm({
      title: `Supprimer ${ids.length} élément(s)`, message: 'Cette action est irréversible.',
  ...
  toggleActif(item: StatutStage, e: Event): void {
    e.stopPropagation();
    if (!this.perms.canEditStatutStage()) { this.ui.warning('Permission requise.'); return; }
    const req = item.actif ? this.svc.deactivate(item.id) : this.svc.activate(item.id);
    req.subscribe({ next: saved => this.items.update(l => l.map(x => x.id === saved.id ? saved : x)) });
  }
